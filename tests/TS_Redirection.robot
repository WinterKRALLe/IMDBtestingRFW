...
TC_3: Span Link
    • Check if a specific span element can redirect to a page with the title 'What to watch'.
    1. Wait until the page loads the specific span element.
    2. Because it is not optically visible and not clickable, it needs to be scrolled down using a JS command.
    3. The page has to wait until it is scrolled, put it to sleep.
    4. Click on the span element.
    5. Verify if the URL contains the word 'what-to-watch'.
TC_4: Heading Link
    • The page should redirect to the home page and be able to repeat the previous process.
    1. Click on the logo element.
...
TC_5: Menu Link
    • The page should be able to open the menu and contain clickable links.
...
TC_6: Image Link
    ...
    • Clicking on an image element should redirect to a specific page.
    1. Click on the image element.
    2. Verify if the page contains the text 'Top 50 Crime Movies and TV Shows'.
TC_7: Icon Link
    • Now the page should contain a list of 50 movies.
    • Every movie has a title, an image, rating, and clickable icon 'Add to Watchlist'.
    1. Wait until the page contains the first clickable icon.
    2. Give it some time to load.
    3. Click on the element.
    4. The user is not logged in, so it redirects to the signin page that should contain the title 'Sign in'.
